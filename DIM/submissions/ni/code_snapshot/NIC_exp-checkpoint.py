...
if train:
    ...
    for i, train_batch in enumerate(dataset):
        ...
        if i == 0:
            ...
            if i%4==1:
                dataP = ext_mem[0]## dataP vector can be removed and we can insert directly into concat
                labP = ext_mem[1]
                if replay:
                ...
                dataC = np.concatenate((data,dataC),axis=0)
                labC = np.concatenate((labels,labC),axis=0)
...
#                 labC = np.concatenate((labels[index_tr],labels[index_cv]),axis=0)
            ...
            ### add replay here   
            index_tr,index_cv,coreset = data_split(data.shape[0],777)
            ext_mem = [ np.concatenate((data[coreset], ext_mem[0])),
                 np.concatenate((labels[coreset], ext_mem[1]))]
