...
class NI_wrap():
        ...
        for i, train_batch in enumerate(self.dataset):
            if i==0:
                ...
                if i==0 and self.load:
                    ...
                    dim_model.load_state_dict(torch.load(self.path + 'weights/weightsDIM_T0cset256.pt'))
                else:
                    ############################## Train Encoder########################################
                    dim_model,self.stats = trainEnc_MI(self.stats,dim_model, optimizer, scheduler,dataloaders,self.device,tr_dict_enc)
                    ####################################################################################
                    torch.save(dim_model.state_dict(), self.path + 'weights/weightsDIM_T'+str(i)+'cset256.pt')
                ...
                torch.save(classifierM.state_dict(), self.path + 'weights/weightsC_T'+str(i)+'cset256.pt')
    ...
    def test(self,test_data,standalone=False):
        if stanalone:
            ...
            classifierM = classifier(n_input = 128,n_class=50)
            classifierM = classifierM.to(self.device)   
            dim_model.load_state_dict(torch.load(self.path + 'weights/weightsDIM_T7cset256.pt'))
            classifierM.load_state_dict(torch.load(self.path + 'weights/weightsC_T7cset256.pt'))

        else:
            test_set = LoadDataset(self.test_data,transform=None)
            batch_size=100
            test_loader = DataLoader(test_set, batch_size=batch_size, shuffle=False)
            score= []
            self.dim_model.eval()
            ...
            for inputs in test_loader:
                ...
                score.append(np.argmax(pred_l,axis=1))
