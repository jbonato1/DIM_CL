...
#     use_cutmix = kwargs['Cutmix_Flag']
#     cutmix_prob = kwargs['cutmix_prob']
#     cutmix_beta = kwargs['cutmix_beta']
                ...
                with torch.set_grad_enabled(phase == 'train'):                    
                    ...
                    if use_cutmix:
                        ...
                        if cutmix_beta > 0 and r < cutmix_prob:
                            ...
                            lam = 1 - ((bbx2 - bbx1) / inputs.size()[-1] )

                            # compute output
                            out = model(inputs)
                            loss = 0
                            loss = compute_loss(out, labels_a,metrics) * lam + compute_loss(out, labels_b,metrics) * (1. - lam)
                    else:  
                        out = model(inputs)
                        loss = 0   
